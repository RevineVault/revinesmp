...
// ====================================================================
// FITUR CLAIM RANK & CUSTOM ALERT REGION
// ====================================================================
function showClaimAlert(title, message, type = 'success', callback = null) {
    const alertModal = document.getElementById('claim-alert-modal');
    const alertIcon = document.getElementById('claim-alert-icon');
    const alertTitle = document.getElementById('claim-alert-title');
    const alertMessage = document.getElementById('claim-alert-message');
    const alertBtn = document.getElementById('btn-alert-action');
    ...
    // Atur tipe icon & warna tombol popup
    if (type === 'error') {
        alertIcon.className = 'claim-alert-icon error';
        alertIcon.innerHTML = '<i class="fas fa-times-circle"></i>';
        alertBtn.style.background = '#ef4444';
        alertBtn.innerText = 'Coba Lagi';
    } else {
        alertIcon.className = 'claim-alert-icon success';
        alertIcon.innerHTML = '<i class="fas fa-check-circle"></i>';
        alertBtn.style.background = '#10b981';
        alertBtn.innerText = 'Lanjut ke WA';
    }
    
    alertModal.classList.add('show');
    
    // Logika tombol popup diklik
...
function openClaimModal(rankName = '') {
    const modal = document.getElementById('claim-modal');
    if(modal) {
        modal.classList.add('show');
        if(rankName) {
            document.getElementById('claim-rank').value = rankName;
        }
    }
}
...
function submitClaim() {
    const ign = document.getElementById('claim-ign').value;
    const name = document.getElementById('claim-name').value;
    const wa = document.getElementById('claim-wa').value;
    const rank = document.getElementById('claim-rank').value;
    const proof = document.getElementById('claim-proof').value;

    // Validasi form kosong menggunakan Custom Alert Error
    if(!ign || !name || !wa) {
        showClaimAlert(
            "Data Belum Lengkap!", 
            "Harap isi semua kolom form (Username, Nama, dan No WA) terlebih dahulu agar admin bisa memproses data kamu.", 
            "error"
        );
        return;
    }

    // Nomor Admin Revine SMP (Sesuai database web)
    const adminWA = "6283898777946"; // Ganti dengan nomor WA admin yang benar

    // Format Pesan ke WA
    let text = `Halo Admin Revine SMP, saya ingin melakukan Claim Rank.%0A%0A`;
    text += `*Data Claim:*%0A`;
    text += `- Username MC: ${ign}%0A`;
    text += `- Nama: ${name}%0A`;
    text += `- No WA: ${wa}%0A`;
    text += `- Rank: ${rank}%0A`;
    
    if(proof) {
        text += `%0A*(Saya ada bukti struk/SS, fotonya akan saya kirimkan manual di chat ini)*`;
    } else {
        text += `%0A*(Saya tidak ganti username / no WA lama)*`;
    }

    const waUrl = `https://wa.me/${adminWA}?text=${text}`;

    // Tampilkan Custom Alert Sukses dulu, pas tombol diklik baru redirect ke WA
    showClaimAlert(
        "Form Valid!", 
        "Data kamu berhasil disiapkan. Klik tombol di bawah untuk otomatis beralih ke WhatsApp Admin.", 
        "success", 
        function() {
